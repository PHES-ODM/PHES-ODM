-- ============================================================================
-- PHES-ODM v2.3.0 (archival) — MySQL / MariaDB schema
--
...
-- was already underway. Derived from schema-mysql-v3.0.1.sql by removing
...
--     meaningless once that table is gone) and its now-vestigial
--     chk_measures_epiWeek/epiYear CHECK constraints.
--   - `sites`: dropped siteLevel.
--   - `samples`: dropped collAppxT/collDate/epiWeek/epiWeekStart/epiYear
--     and its now-vestigial chk_samples_epiWeek/epiYear CHECK constraints.
...
-- v3.0.1 lacks. Load-tested clean against a real MariaDB database.
--
...
-- schema-mysql-v3.0.1.sql, unchanged below.
--
...
CREATE TABLE `parts` (
  ...
  `fKAliasID`       VARCHAR(30)   CHARACTER SET ascii NOT NULL,   -- FK -> parts (partID reference by convention); "NA" sentinel when not applicable, verified never blank across all 2345 rows
  ...
  `dataType` VARCHAR(30)    CHARACTER SET ascii NOT NULL,  -- FK -> parts (dataTypes enum)
...
ALTER TABLE `parts` ADD CONSTRAINT `fk_parts_fKAliasID`     FOREIGN KEY (`fKAliasID`)        REFERENCES `parts`(`partID`);
